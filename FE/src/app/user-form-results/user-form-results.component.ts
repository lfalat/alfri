...
import {
  ...
  CategoryScale,
  Chart as chartJs,
  Chart, ChartDataset, Filler, Legend,
  LinearScale,
  LineElement,
  PointElement, RadarController, RadialLinearScale,
  TimeScale,
  Title, Tooltip
} from 'chart.js';
...
export class UserFormResultsComponent implements OnInit, AfterViewInit {
  ...
  dataSource: { subjectName: string; grade: string }[] = [];
  ...
  ngOnInit(): void {
    ...
    this.dataSource = this.existingAnswers.sections[1].questions.map(
      (question) => {
        return {
