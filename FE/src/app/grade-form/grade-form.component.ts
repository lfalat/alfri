...
export class GradeFormComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.formService.getForm(USER_FORM_ID).pipe(
      switchMap(formData => {
        this.form = formData;
    ...
    this.formGroups = this.form.sections.map((section) => {
      ...
      section.questions.forEach(question => {
        ...
        if (this.existingAnswers) {
          const existingSection = this.existingAnswers.sections.find(s => s.sectionTitle === section.sectionTitle);
            ...
            if (existingQuestion) {
              if (question.answerType === 'CHECKBOX' && question.options) {
                ...
                question.options.forEach((option, index) => {
                  const isChecked = existingQuestion.answers.some(
                    answer => answer.texts.some(answer => answer.textOfAnswer === option.questionOption)
                  );
      ...
      this.formGroups.forEach((group, index) => {
        ...
        section.questions.forEach((question) => {
          ...
          if (question.answerType === 'CHECKBOX') {
            ...
            answer.texts.push({ textOfAnswer: group.get(question.questionIdentifier)?.value });
          }
          answers.push(answer);
        });
      });
  ...
  nextSection($event: StepperSelectionEvent) {
    if (this.form.sections[$event.selectedIndex].sectionTitle === 'Oblasti záujmov') {
      if (!this.radarChart) {
  ...
  initializeRadarChart(index: number): void {
    ...
    if (value) {
      ...
      this.chartDatasets[0].data = Object.values(Object.fromEntries(
        Object.keys(focusLabelMapping).map(key => [key, this.formGroups[index].value[key]])
      )) as number[];
