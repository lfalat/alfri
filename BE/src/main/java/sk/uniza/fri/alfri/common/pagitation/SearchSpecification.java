...
public class SearchSpecification<T> implements Specification<T> {
    @Serial
    private static final long serialVersionUID = -9120811319573849240L;
    ...
    private void addPredicateFromSearchCriteria(SearchCriteria searchCriteria, List<Predicate> predicates, CriteriaBuilder builder, Path<?> fieldPath) {
        switch (searchCriteria.getOperation()) {
            ...
            case ":": {
                predicates.add(builder.equal(fieldPath, searchCriteria.getFirstValue(fieldPath.getJavaType())));
                break;
            }
            case "~": {
